...
fastify.post(
    ...
    async (
        ...
        // Validate that the request ID is present and exists
        if (!id || !requests[id]) {
            return reply.status(404).send({
                error: 'invalid_request',
                details: 'Request ID not found',
            });
        }

        // Handle unencrypted error responses (allowed by OAuth 2.0 spec)
        if (error && !response) {
            // Validate that the state matches before accepting the error
            if (state && state !== requests[id].state) {
                return reply.status(400).send({
                    error: 'invalid_request',
                    details: 'State mismatch in error response',
                });
            }
            // Use the request ID to identify the request
            requests[id].error = {
                error,
                error_description,
                state: state || requests[id].state,
            };
            return reply.status(200).send({
                redirect_uri: `${BASE_URL}/error?response_code=${requests[id].response_code}`,
            });
        }
        ...
        // Decrypt the HAIP encrypted response using the request ID to get the encryption key
        let decryptedPayload: any = null;

        try {
            decryptedPayload = await decryptResponse(response, requests[id].encryptionPrivateKey);
        } catch (decryptError) {
            return reply.status(400).send({
                error: 'invalid_request',
        ...
        const effectiveState = decryptedPayload.state;
        if (!effectiveState) {
            return reply.status(400).send({
                error: 'invalid_request',
        ...
        // Validate that the state matches the stored request
        if (requests[id].state !== effectiveState) {
            return reply.status(400).send({
                error: 'invalid_request',
                details: 'State mismatch: decrypted state does not match stored request state',
            });
        }

        const matchingRequest = requests[id];
